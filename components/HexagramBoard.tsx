...
export const HexagramBoard: React.FC<HexagramBoardProps> = ({ 
  ...
  if (view === 'grid' || view === 'sequence') {
    return (
      <div className={`grid grid-cols-5 md:grid-cols-8 gap-1.5 md:gap-2 p-3 bg-white/60 rounded-3xl shadow-inner border border-gray-100 mx-auto w-full max-w-2xl`}>
        {activeHexList.map((hex, idx) => {
          ...
          return (
            <div 
              ...
              className={`relative flex flex-col items-center p-1 md:p-1.5 rounded-xl border transition-all cursor-pointer group select-none ${isFinished ? (isCorrect ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200') : 'bg-white border-transparent hover:border-indigo-100'}`}
            >
              <div className="mb-1 pointer-events-none">
                <HexagramIcon lines={hex.lines} size={24} color="#374151" />
              </div>
              <div className={`w-full text-[7px] md:text-[9px] min-h-[1.1rem] flex items-center justify-center rounded transition-all truncate font-bold ${item ? 'bg-indigo-600 text-white' : 'text-gray-200 italic border border-dashed border-gray-50'}`}>
                {item?.content || ''}
              </div>
            </div>
          );
  ...
  return (
    <div className="relative w-[95vmin] h-[95vmin] max-w-[800px] max-h-[800px] mx-auto flex items-center justify-center select-none">
      ...
      {circularSequence.map((hexId, step) => {
        ...
        return (
          ...
          >
            <div onClick={() => onSlotClick(hexId, 'hexName')} onDoubleClick={(e) => { e.stopPropagation(); onHexClick?.(hex); }} className="cursor-pointer">
               <HexagramIcon lines={hex.lines} size={20} color="#374151" />
            </div>
            {item && (
               <div className="text-[6px] md:text-[8px] px-1 py-0.5 rounded-sm bg-indigo-600 text-white font-black whitespace-nowrap shadow-md">
                 {item.content}
               </div>
            )}
          </div>
